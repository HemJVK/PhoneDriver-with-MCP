...
# Initialize the logger globally, once.
if 'log_handler' not in st.session_state:
    root_logger = logging.getLogger()
    if root_logger.hasHandlers():
        root_logger.handlers.clear()
    
    st.session_state.log_handler = UILogHandler()
...
with query_tab:
    ...
    if st.button("Execute Task") and query:
        ...
        if review_mode:
            ...
            st.session_state.run_approved = True # Skip review and run directly
    ...
    if st.session_state.run_approved:
        ...
        with st.spinner("Executing plan..."):
            ...
            result = st.session_state.agent.agent.run_task_loop(query, plan=plan_to_run)
