...
class VisionAnalyzer:
    ...
    def __init__(self, model_name: str = "HuggingFaceTB/SmolVLM2-2.2B-Instruct"):
        ...
        self.logger.info(
            f"Initializing VisionAnalyzer with SmolVLM on device: {self.device}"
        )
    ...
    def describe_screen(self, screenshot_path: str) -> str:
        ...
        try:
            ...
            inputs = self.processor(prompt, images=[raw_image], return_tensors="pt").to(
            ...
            self.logger.error(
                f"Failed to describe screen with SmolVLM model: {e}", exc_info=True
            )
